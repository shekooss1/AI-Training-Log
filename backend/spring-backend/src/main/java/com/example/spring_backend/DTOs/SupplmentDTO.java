...
public record SupplmentDTO(Long id,String name, double dose, double time,String notes) {
public static SupplmentDTO from(Supplment s){
    return new SupplmentDTO(s.getId(),s.getName(),s.getDose(),s.getTime(),s.getNotes());
}
